...
export type AppStoreState = {
  ...
  /** 段落书签：按书 / 段落 id 列表（用户长按菜单 → 标记） */
  bookmarksByBook: Record<string, string[]>;
  toggleBookmark: (bookId: string, paragraphId: string) => void;
...
export const useAppStore = create<AppStoreState>()(
  persist(
    (set, get) => ({
      ...
      bookmarksByBook: {},
      toggleBookmark: (bookId, paragraphId) =>
        set((s) => {
          const list = s.bookmarksByBook[bookId] ?? [];
          const next = list.includes(paragraphId)
            ? list.filter((id) => id !== paragraphId)
            : [...list, paragraphId];
          return {
